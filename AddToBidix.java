...

{
...

	{
		...
		while (true)
		{
			...
			lemma = Inmatning.rad("Lemma (grundform) " + tmLang.get(left) + ":", stop, "Ordet finns redan i ordlistan. ***\nå æ ø - å ä ö ").trim();
		...
		else
		{
			...
			lemma = Inmatning.rad("Lemma (grundform) " + tmLang.get(right) + ":").trim();
			lemma = lemma.replaceAll(" ","<b/>");
			ord = ord + "|" + lemma;
	...
	String nyttOrd = "";	
		...
		{	
			...
			//if (rad.contains("<e lm=") && rad.contains("__" + klass))
			if (rad.contains("<p><l>") && rad.contains("n=\"" + klass)) // Definition av ord i rätt klass
